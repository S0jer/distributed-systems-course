...
async def create_pool(pool: Pool):
    ...
    pools[pool_id] = pool
    return {"pool_id": pool_id, **pool.dict()}
...
@app.post("/pool/{pool_id}")
async def pool(
        pool_id: str
):
    return JSONResponse(status_code=status.HTTP_200_OK, content=pool_id)
...
async def add_vote(pool_id: str, vote: Vote):
    if pool_id not in pools:
        raise HTTPException(status_code=404, detail="Pool not found")

    pools[pool_id].answers.append(vote)
    return {"message": "Vote added successfully"}


@app.post("/pool/{pool_id}/vote/{vote_id}")
async def add_vote(pool_id: str, vote: Vote):
    if pool_id not in pools:
        raise HTTPException(status_code=404, detail="Pool not found")
